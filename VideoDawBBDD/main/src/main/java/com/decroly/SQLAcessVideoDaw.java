...
public class SQLAcessVideoDaw {
    ...
    public List<Articulo> getVideojuegos(){//METODO PARA MOSTRAR TODOS LOS ARTICULOS
        List<Articulo> Videojuegos = new LinkedList<>();

        String getVideojuegos =   "SELECT a.cod, a.titulo, a.descripcion, a.fecha_registro, p.genero, p.alquilada FROM Articulo a JOIN Pelicula p ON p.cod = a.cod WHERE a.fecha_baja = '9999-01-01';";

        try (Connection connection = SQLDatabaseManager.getConnection(); Statement statement = connection.createStatement();
        ResultSet dataSet = statement.executeQuery(getVideojuegos);) {
            while(dataSet.next()){
                int cod = dataSet.getInt(1);
                String titulo = dataSet.getNString(2);
                String descripcion = dataSet.getNString(3);
